...
adviceEdgelist <- read.csv("edge_list.csv")
...
#plot(giantGraph, vertex.size = 0.1, vertex.label = NA, edge.arrow.size=.01) ## review the largest component


# Independent variables:
# Load node attributes, and store them in the advice network object we have created
#set.vertex.attribute(advice, "views",read.csv("views.csv",stringsAsFactors=FALSE)$views) # Categorical variable for department
set.vertex.attribute(advice, "mature",read.csv("mature.csv")$mature) # Indicator variable for department leader
set.vertex.attribute(advice, "life_time",read.csv("life_time.csv")$life_time) # Years tenure
set.vertex.attribute(advice, "affiliate",read.csv("affiliate.csv")$affiliate) # Indicator variable for whether they are located in the main or secondary office
set.vertex.attribute(advice, "language",read.csv("language.csv")$language)
...
get.vertex.attribute(advice,"views")
get.vertex.attribute(advice,"mature")
get.vertex.attribute(advice,"life_time")
get.vertex.attribute(advice,"affiliate")
get.vertex.attribute(advice,"language")
...
summary(advice ~ nodeicov("affiliate"))             # Ties directed towards employees at the main office (as opposed to secondary office)
summary(advice ~ nodeocov("affiliate"))             # Ties originating from employees at the main office (as opposed to secondary office)
summary(advice ~ nodematch("mature"))            # Number of ties between people of the same sex
summary(advice ~ nodematch("language"))        # Number of ties between people working in the same department
summary(advice ~ diff("views"))                 # Total difference in tenure: sum of (sender's tenure - receivers-tenure) for all ties
summary(advice ~ diff("life_time"))
                                              # 
...
library(igraph)
advice_igraph <- graph.adjacency(as.matrix.network(advice)) # make an igraph network object from statnet network object
comp <- components(advice_igraph)
is_directed(advice_igraph)
giantGraph <- advice_igraph %>% 
  induced.subgraph(., which(comp$membership == which.max(comp$csize)))
giant_obj <- as_data_frame(giantGraph)
summary(giant_obj)
...
model1 <- ergm(giant_obj ~ edges                 # This is  a tendency towards a greater number of advice ties existing. Based on a statistic counting the number of ties.
               # Structural patterns
                                # Inverted preferential attachment (indegree)
                            # Inverted preferential attachment (outdegree)
               ...
               # Model constraints
                # This constraint enforces the maximum outdegree is 5
) 
summary(model1) 
